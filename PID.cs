...
public class RBPID
{
    ...
    public void UpdateVelocity(Vector3 targetPos, float forceMult = 1f)
    {
        if (!isActive)
            return;
        if (Time.deltaTime == 0 || Time.deltaTime.IsInfiniteOrNaN()) return;
    ...
    public void UpdateTorque(Quaternion targetRot, float forceMult = 1f)
    {
        if (!isActive)
            return;
        if (Time.deltaTime == 0 || Time.deltaTime.IsInfiniteOrNaN()) return;
        var rotation = (Vector3.Cross(rb.transform.rotation * Vector3.forward, targetRot * Vector3.forward) + Vector3.Cross(rb.transform.rotation * Vector3.up, targetRot * Vector3.up)).normalized * Quaternion.Angle(rb.transform.rotation, targetRot) / 360;
    ...
    public void StabilizeAngularVelocity(float strength = 5f)
    {
        if (rb == null || !isActive) return;
        Vector3 dampingTorque = -rb.angularVelocity * strength;
